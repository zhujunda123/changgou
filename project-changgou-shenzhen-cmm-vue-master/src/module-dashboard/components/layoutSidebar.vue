<template>
  <div>
    <scroll-bar>
      <!--导航菜单-->
      <el-menu router :collapse="collapsed" ref="leftNavigation" class="leftMenu">
          ...
          <template v-if="issue.name === $store.state.leftNavState">
            ...
            <template v-for="(item,index) in issue.children">
              <el-submenu v-if="!item.leaf" :index="index+''" v-show="item.menuShow">
                <template slot="title" ><i :class="item.iconCls"></i><span slot="title">{{item.title}}</span></template>
